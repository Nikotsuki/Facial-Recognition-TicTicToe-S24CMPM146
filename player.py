...
class UserWebcamPlayer:
    ...
    def _get_emotion(self, img) -> int:
        ...
        sample_list = np.array([rgb])

        # Normalize the image (optional, depending on your model)
        img_normalized = resized / 255.0

        # Expand dimensions to match the model input shape (1, height, width, channels)
        img_input = np.expand_dims(img_normalized, axis=0)

        # Load the pre-trained model
        model = models.load_model("results/basic_model_15_epochs_timestamp_1723184559.keras")
        #models._compile_model(model)

        if len(img_input.shape) == 3:
            img_input = np.expand_dims(img_input, axis=-1)  # Add a channel dimension
